...
class OrderControllerTest {
    ...
    class CreateOrderTests {
        ...
        @Test
        @DisplayName("Should create order, return 201, calculate total, and deduct inventory")
        void createOrder_happyPath() throws Exception {
            int initialQty = productRepository.findById(1).get().getQuantity();

            String response = mockMvc.perform(post("/api/orders")
                    .with(httpBasic("admin_1", ""))
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"user_id\":3,\"products\":[{\"productId\":1,\"quantity\":2}]}"))
            ...
            JSONAssert.assertEquals(expected, response, JSONCompareMode.LENIENT);
            assertEquals(initialQty - 2, productRepository.findById(1).get().getQuantity());
        }

        @Test
        @DisplayName("Should return 403 when non-admin tries to create order")
        void createOrder_asNonAdmin_returns403() throws Exception {
        ...
        void createOrder_productNotFound_returns404() throws Exception {
            mockMvc.perform(post("/api/orders")
                    .with(httpBasic("admin_1", ""))
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"user_id\":3,\"products\":[{\"productId\":999,\"quantity\":1}]}"))
                    .andExpect(status().isNotFound());
        }
        ...
        void createOrder_insufficientStock_returns400() throws Exception {
            mockMvc.perform(post("/api/orders")
                    .with(httpBasic("admin_1", ""))
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"user_id\":3,\"products\":[{\"productId\":3,\"quantity\":10}]}"))
