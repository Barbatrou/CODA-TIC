...
class ProductControllerTest {
    ...
    class AdjustCategoryPricesTests {
        ...
        @Test
        @DisplayName("Should adjust prices, return 200, and persist changes")
        void adjustPrices_happyPath() throws Exception {
            mockMvc.perform(patch("/api/products/category/book/price?percentage=10")
                    .with(httpBasic("admin_1", "")))
                    .andExpect(status().isOk());

            assertEquals(8.79, productRepository.findById(1).get().getPrice());
        }

        @Test
        @DisplayName("Should return 403 when non-admin tries to adjust prices")
        void adjustPrices_asNonAdmin_returns403() throws Exception {
            mockMvc.perform(patch("/api/products/category/book/price?percentage=10")
                    .with(httpBasic("client_1", "")))
                    .andExpect(status().isForbidden());
        }
        ...
        void adjustPrices_categoryNotFound_returns404() throws Exception {
            mockMvc.perform(patch("/api/products/category/electronics/price?percentage=10")
                    .with(httpBasic("admin_1", "")))
                    .andExpect(status().isNotFound());
        }

        @Test
        @DisplayName("Should return 400 when percentage is invalid")
        void adjustPrices_invalidPercentage_returns400() throws Exception {
            mockMvc.perform(patch("/api/products/category/book/price?percentage=-100")
                    .with(httpBasic("admin_1", "")))
